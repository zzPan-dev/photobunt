...
// ---- Local patch (2026): keyboard navigation on modern browsers. ----
// The original keyHandler switches on e.keyCode only; normalize it from
// e.key when a browser/event source does not populate the legacy field.
(function () {
	var map = { ArrowRight: 39, Right: 39, PageDown: 34,
	            ArrowLeft: 37, Left: 37, PageUp: 33,
	            ArrowDown: 40, Down: 40, ArrowUp: 38, Up: 38,
	            Escape: 27, Esc: 27, Enter: 13 };
	var orig = hs.keyHandler;
	hs.keyHandler = function (e) {
		if (!e) e = window.event;
		var code = e.keyCode || map[e.key] || 0;
		return orig({ keyCode: code, target: e.target || e.srcElement });
	};
})();
